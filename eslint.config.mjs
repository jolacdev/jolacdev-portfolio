...
const compat = new FlatCompat({
  baseDirectory: import.meta.dirname, // `import.meta.dirname` is available after Node v20.11.0
});

// Disable all `eslint-plugin-import` rules since they are automatically imported by `eslint-config-next`.
const eslintPluginImportRulesOff = Object.keys(eslintPluginImport.rules).reduce(
  (acc, rule) => {
    acc[`import/${rule}`] = 0;
    return acc;
  },
...
export default [
  ...
  // NOTE: Backward compatibility for Next.js linting configuration.
  ...compat.config({
    extends: ['next/core-web-vitals', 'next/typescript'],
    rules: {
      ...eslintPluginImportRulesOff,
    },
  }),
  ...
  prettierRecommended,
  // NOTE: Already included by `eslint-config-next`.
...

  {
    ...
    rules: {
      ...
      // Enforces not using console.log, console.error, etc. in the code.
      'no-console': 2,
